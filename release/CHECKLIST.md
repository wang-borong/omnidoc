...
1. Set the version once; the command updates `Cargo.toml`, `Cargo.lock`,
   ...
   ```bash
   python3 scripts/set-version.py 1.9.5
   ```
...
```bash
OMNIDOC_RELEASE_TAG=v1.9.5 bundles/libs/scripts/package-release.sh /tmp/omnidoc-libs-a
OMNIDOC_RELEASE_TAG=v1.9.5 bundles/libs/scripts/package-release.sh /tmp/omnidoc-libs-b
...
1. Commit, create, and push the single product tag, for example `v1.9.5`.
...
3. Confirm the GitHub release contains every binary package plus
   `omnidoc-libs-v1.9.5.tar.gz` and its `.sha256` file. Each platform binary
